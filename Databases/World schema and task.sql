-- Task Answers
    ...
    SELECT Country.Name AS 'Country Name', City.Name AS 'Capital City', City.Population AS 'Cap. City Population' 
    FROM Country
    INNER JOIN City
	...
	SELECT Country.Name AS 'Country Name', CountryLanguage.Language AS 'Language' 
    FROM Country
    INNER JOIN CountryLanguage
    ON Country.code=CountryLanguage.CountryCode -- AND isofficial='T';
    WHERE CountryLanguage.IsOfficial = 'T';
    
    -- select country name, city name which have more than 3 cities

SELECT Country.name AS 'Country Name', City.name AS 'City Name'
FROM Country
...
JOIN city ON c.countrycode = city.countrycode
ORDER BY country.name;
USE WORLD;
    -- select country name, city name which have more than 3 cities
SELECT country.name AS 'Country Name', 
       GROUP_CONCAT(city.name SEPARATOR ', ') AS 'City Names'
FROM city
...
ORDER BY country.name;
-- OR
SELECT co.Name AS 'Country', ci.Name AS 'City' FROM Country co, City ci WHERE ci.CountryCode=co.Code 
AND co.Code IN (SELECT ci.CountryCode FROM city ci GROUP BY ci.CountryCode
HAVING COUNT(ci.CountryCode)>3);


    SELECT countrycode, count(city.countrycode) FROM city
    GROUP BY city.countrycode;

-- select those countries and their languages which have more than 2 official languages

SELECT Country.Name AS 'Country Name', GROUP_CONCAT(CountryLanguage.Language SEPARATOR ', ') AS 'Languages'
FROM Country
...
-- List all countries name which are located in Asia and thier name ends with N?

SELECT Name FROM Country WHERE Name LIKE '%n' AND continent = 'Asia';
